...
.setAction(async function ({ token, spender, deadline }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  const klp = erc20.attach(token)   
  
  await (await klp.connect(await getNamedSigner("dev")).approve(spender, deadline)).wait()    
});
...
task("migrate", "Migrates liquidity from Uniswap to Kanga")
...
task("masterbreeder:add", "Add pool to masterbreeder")
.setAction(async function (taskArguments, { ethers: { getNamedSigner } }, runSuper) {
  ...
  await (await masterBreeder.connect(await getNamedSigner("dev")).add(1000, '0x3e78a806b127c02b54419191571d9379819e989c', true)).wait()
});

task("masterbreeder:deposit", "MasterBreeder deposit")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const masterBreeder = await ethers.getContract("MasterBreeder")

  const { lpToken } = await masterBreeder.poolInfo(pid)

  await run("erc20:approve", { token: lpToken, spender: masterBreeder.address })

  await (await masterBreeder.connect(await getNamedSigner("dev")).deposit(pid, amount)).wait()
});

task("masterbreeder:withdraw", "MasterBreeder withdraw")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const masterBreeder = await ethers.getContract("MasterBreeder")
  ...
  await (await masterBreeder.connect(await getNamedSigner("dev")).withdraw(pid, amount)).wait()
});

task("billabong:enter", "Billabong enter")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  await run("erc20:approve", { token: kanga.address, spender: billabong.address })
  
  await (await billabong.connect(await getNamedSigner("dev")).enter(amount)).wait()
});

task("billabong:leave", "Billabong leave")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  const kanga = await ethers.getContract("KangaToken")
  ...
  await (await billabong.connect(await getNamedSigner("dev")).leave(amount)).wait()
});

task("maker:serve", "Billabong serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
