/*
 * This is a specification file for MasterBreederV2's formal verification
 * using the Certora prover.
 *
 * Run this file with scripts/_runMasterBreederV2.sh
 */
...
using DummyKANGA as kangaToken
...
methods {
	...
	poolInfoAccKangaPerShare(uint256 pid) returns (uint128) envfree
	...
	// KANGA token
	KANGA() returns (address) envfree

	// Rewarder
	onKangaReward(uint256, address, address, uint256, uint256) => NONDET
...
rule monotonicityOfAccKangaPerShare(uint256 pid, method f) {
	env e;

	uint128 _poolInfoAccKangaPerShare = poolInfoAccKangaPerShare(pid);

	calldataarg args;
	f(e, args);

	uint128 poolInfoAccKangaPerShare_ = poolInfoAccKangaPerShare(pid);

	assert compareUint128(poolInfoAccKangaPerShare_, _poolInfoAccKangaPerShare);
}
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	uint128 _otherAccKangaPerShare = poolInfoAccKangaPerShare(otherPid);
	...
	uint128 otherAccKangaPerShare_ = poolInfoAccKangaPerShare(otherPid);
	uint64 otherLastRewardBlock_ = poolInfoLastRewardBlock(otherPid);
	uint64 otherAllocPoint_ = poolInfoAllocPoint(otherPid);

	assert(_otherAccKangaPerShare == otherAccKangaPerShare_, "accKangaPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require KANGA() != lpToken(pid); // <-- check this again (Nurit)
...
rule kangaGivenInHarvestEqualsPendingKanga(uint256 pid, address user, address to) {
	env e;

	require to == user && user != currentContract && e.msg.sender == user;
	require kangaToken == KANGA();

	uint256 userKangaBalance = kangaToken.balanceOf(e, user);
	uint256 userPendingKanga = pendingKanga(e, pid, user);

	// Does success return value matters? Check with Nurit
	harvest(e, pid, to);

	uint256 userKangaBalance_ = kangaToken.balanceOf(e, user);

	assert(userKangaBalance_ == (userKangaBalance + userPendingKanga),
		   "pending kanga not equal to the kanga given in harvest");
}
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingKanga(uint256, address).selector) {
		pendingKanga(e, pid, to);
