...
describe("Migrator", function () {
  ...
  beforeEach(async function () {
    ...
    this.breeder = await this.MasterBreeder.deploy(this.kanga.address, this.dev.address, "1000", "0", "100000")
    await this.breeder.deployed()

    this.migrator = await this.Migrator.deploy(this.breeder.address, this.factory1.address, this.factory2.address, "0")
    await this.migrator.deployed()

    await this.kanga.transferOwnership(this.breeder.address)

    await this.breeder.add("100", this.lp1.address, true)
  })
  ...
  it("should do the migration successfully", async function () {
    ...
    await this.lp1.sync()
    await this.lp1.connect(this.minter).approve(this.breeder.address, "100000000000", { from: this.minter.address })
    await this.breeder.connect(this.minter).deposit("0", "2000000", { from: this.minter.address })
    expect(await this.lp1.balanceOf(this.breeder.address), "2000000")
    await expect(this.breeder.migrate(0)).to.be.revertedWith("migrate: no migrator")

    await this.breeder.setMigrator(this.migrator.address)
    await expect(this.breeder.migrate(0)).to.be.revertedWith("migrate: bad")

    await this.factory2.setMigrator(this.migrator.address)
    await this.breeder.migrate(0)
    expect(await this.lp1.balanceOf(this.breeder.address)).to.equal("0")
    expect(await this.lp2.balanceOf(this.breeder.address)).to.equal("2000000")

    await this.breeder.connect(this.minter).withdraw("0", "2000000", { from: this.minter.address })
