...
// Version: 22-Feb-2021
// Modifications Copyright (c) 2021 KangaFinance
// 2021-09 : Customize for Kanga Finance
// Troop Lending
...
// File @kanga/kangamob-sdk/contracts/IBatchFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @kanga/kangamob-sdk/contracts/IFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @kanga/kangamob-sdk/contracts/IStrategy.sol@v1.0.1
// License-Identifier: MIT
...
// File @kanga/kangswao-sdk/contracts/IKangaMobV1.sol@v1.0.1
// License-Identifier: MIT

interface IKangaMobV1 {
...
interface ISwapper {
    /// @notice Withdraws 'amountFrom' of token 'from' from the KangaMob account for this swapper.
    /// Swaps it for at least 'amountToMin' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the KangaMob using a plain ERC20 transfer.
    /// Returns the amount of tokens 'to' transferred to KangaMob.
    /// (The KangaMob skim function will be used by the caller to get the swapped funds).
    function swap(
        IERC20 fromToken,
        IERC20 toToken,
        address recipient,
    ...
    /// Withdraws 'amountFrom' of token 'from' from the KangaMob account for this swapper.
    /// Swaps it for exactly 'exactAmountTo' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the KangaMob using a plain ERC20 transfer.
    /// Transfers allocated, but unused 'from' tokens within the KangaMob to 'refundTo' (amountFromMax - amountFrom).
    /// Returns the amount of 'from' tokens withdrawn from KangaMob (amountFrom).
    /// (The KangaMob skim function will be used by the caller to get the swapped funds).
    function swapExact(
        IERC20 fromToken,
        IERC20 toToken,
...
// File contracts/TroopPair.sol
// License-Identifier: UNLICENSED
// Troop Lending Medium Risk

/// @title TroopPair
/// @dev This contract allows contract calls to any contract (except KangaMob)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract TroopPairMediumRiskV1 is ERC20, BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    IKangaMobV1 public immutable kangaMob;
    TroopPairMediumRiskV1 public immutable masterContract;
    ...
    Rebase public totalAsset; // elastic = KangaMob shares held by the TroopPair, base = Total fractions held by asset suppliers
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Troop Medium Risk ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    // Settings for the Medium Risk TroopPair
    ...
    constructor(IKangaMobV1 kangaMob_) public {
        kangaMob = kangaMob_;
        masterContract = this;
        feeTo = msg.sender;
    }
    ...
    function init(bytes calldata data) public payable override {
        require(address(collateral) == address(0), "TroopPair: already initialized");
        (collateral, asset, oracle, oracleData) = abi.decode(data, (IERC20, IERC20, IOracle, bytes));
        require(address(collateral) != address(0), "TroopPair: bad pair");
    ...
    function accrue() public {
        ...
        uint256 fullAssetAmount = kangaMob.toAmount(asset, _totalAsset.elastic, false).add(_totalBorrow.elastic);
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, false, exchangeRate), "TroopPair: user insolvent");
    }
    ...
    /// False if tokens from msg.sender in `kangaMob` should be transferred.
    function _addTokens(
        IERC20 token,
        uint256 share,
        uint256 total,
        ...
        if (skim) {
            require(share <= kangaMob.balanceOf(token, address(this)).sub(total), "TroopPair: Skim too much");
        } else {
            kangaMob.transfer(token, msg.sender, address(this), share);
        }
    }
    ...
    function addCollateral(
        ...
        emit LogAddCollateral(skim ? address(kangaMob) : msg.sender, to, share);
    }
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        kangaMob.transfer(collateral, address(this), to, share);
    }
    ...
    function _addAsset(
        ...
        uint256 allShare = _totalAsset.elastic + kangaMob.toShare(asset, totalBorrow.elastic, true);
        ...
        emit LogAddAsset(skim ? address(kangaMob) : msg.sender, to, share, fraction);
    }
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        require(_totalAsset.base >= 1000, "Troop: below minimum");
        totalAsset = _totalAsset;
        emit LogRemoveAsset(msg.sender, to, share, fraction);
        kangaMob.transfer(asset, address(this), to, share);
    }
    ...
    function _borrow(address to, uint256 amount) internal returns (uint256 part, uint256 share) {
        ...
        share = kangaMob.toShare(asset, amount, false);
        Rebase memory _totalAsset = totalAsset;
        require(_totalAsset.base >= 1000, "Troop: below minimum");
        _totalAsset.elastic = _totalAsset.elastic.sub(share.to128());
        totalAsset = _totalAsset;
    ...
    function _repay(
        ...
        uint256 share = kangaMob.toShare(asset, amount, true);
        uint128 totalShare = totalAsset.elastic;
        _addTokens(asset, share, uint256(totalShare), skim);
        totalAsset.elastic = totalShare.add(share.to128());
        emit LogRepay(skim ? address(kangaMob) : msg.sender, to, amount, part);
    }
    ...
    uint8 internal constant ACTION_MOB_DEPOSIT = 20;
    uint8 internal constant ACTION_MOB_WITHDRAW = 21;
    uint8 internal constant ACTION_MOB_TRANSFER = 22;
    uint8 internal constant ACTION_MOB_TRANSFER_MULTIPLE = 23;
    uint8 internal constant ACTION_MOB_SETAPPROVAL = 24;
    ...
    /// @dev Helper function for depositing into `kangaMob`.
    function _mobDeposit(
        bytes memory data,
        uint256 value,
        uint256 value1,
        uint256 value2
        ...
        return kangaMob.deposit{value: value}(token, msg.sender, to, uint256(amount), uint256(share));
    }

    /// @dev Helper function to withdraw from the `kangaMob`.
    function _mobWithdraw(
        bytes memory data,
        uint256 value1,
        uint256 value2
    ) internal returns (uint256, uint256) {
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return kangaMob.withdraw(token, msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }

    /// @dev Helper function to perform a contract call and eventually extracting revert messages on failure.
    /// Calls to `kangaMob` are not allowed for obvious security reasons.
    ...
    function _call(
        ...
        require(callee != address(kangaMob) && callee != address(this), "TroopPair: can't call");

        (bool success, bytes memory returnData) = callee.call{value: value}(callData);
        require(success, "TroopPair: call failed");
    ...
    /// Only applicable to `ACTION_CALL`, `ACTION_MOB_DEPOSIT`.
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                require((!must_update || updated) && rate > minRate && (maxRate == 0 || rate > maxRate), "TroopPair: rate not ok");
            } else if (action == ACTION_MOB_SETAPPROVAL) {
                (address user, address _masterContract, bool approved, uint8 v, bytes32 r, bytes32 s) =
                    abi.decode(datas[i], (address, address, bool, uint8, bytes32, bytes32));
                kangaMob.setMasterContractApproval(user, _masterContract, approved, v, r, s);
            } else if (action == ACTION_MOB_DEPOSIT) {
                (value1, value2) = _mobDeposit(datas[i], values[i], value1, value2);
            } else if (action == ACTION_MOB_WITHDRAW) {
                (value1, value2) = _mobWithdraw(datas[i], value1, value2);
            } else if (action == ACTION_MOB_TRANSFER) {
                (IERC20 token, address to, int256 share) = abi.decode(datas[i], (IERC20, address, int256));
                kangaMob.transfer(token, msg.sender, to, _num(share, value1, value2));
            } else if (action == ACTION_MOB_TRANSFER_MULTIPLE) {
                (IERC20 token, address[] memory tos, uint256[] memory shares) = abi.decode(datas[i], (IERC20, address[], uint256[]));
                kangaMob.transferMultiple(token, msg.sender, tos, shares);
                ...
                value1 = kangaMob.toShare(asset, totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory kangaMobTotals = kangaMob.totals(collateral);
        ...
        require(allBorrowAmount != 0, "TroopPair: all are solvent");
        ...
        uint256 allBorrowShare = kangaMob.toShare(asset, allBorrowAmount, true);

        if (!open) {
            // Closed liquidation using a pre-approved swapper for the benefit of the LPs
            require(masterContract.swappers(swapper), "TroopPair: Invalid swapper");

            // Swaps the users' collateral for the borrowed asset
            kangaMob.transfer(collateral, address(this), address(swapper), allCollateralShare);
            swapper.swap(collateral, asset, address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = kangaMob.balanceOf(asset, address(this)).sub(uint256(totalAsset.elastic));
            ...
            kangaMob.transfer(asset, address(this), masterContract.feeTo(), feeShare);
            ...
            kangaMob.transfer(collateral, address(this), swapper == ISwapper(0) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(0)) {
                swapper.swap(collateral, asset, msg.sender, allBorrowShare, allCollateralShare);
            }

            kangaMob.transfer(asset, msg.sender, address(this), allBorrowShare);
